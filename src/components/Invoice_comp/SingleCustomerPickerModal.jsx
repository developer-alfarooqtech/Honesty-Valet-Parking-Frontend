...
const SingleCustomerPickerModal = ({
  isOpen,
  onClose,
  ...
  const [localSelection, setLocalSelection] = useState(selectedCustomer);
  ...
  useEffect(() => {
    if (isOpen) {
      setLocalSelection(selectedCustomer);
    }
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {customers.map((customer) => {
                const isSelected = localSelection?._id === customer._id;
                return (
                  <button
                    key={customer._id}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <input
                        ...
                        readOnly
                        checked={isSelected}
                        className="mt-1 h-5 w-5 text-blue-500 border-gray-300"
                      />
                    </div>
                  </button>
                );
...
export default SingleCustomerPickerModal;
