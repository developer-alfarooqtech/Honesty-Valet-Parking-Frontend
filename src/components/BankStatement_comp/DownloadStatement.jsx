...
import { downloadStatement } from "../../service/bankServices";
...
const createHeader = (dateFrom, dateTo) => {
  ...
  title.textContent = "Bank Transactions Report";

  const subtitle = document.createElement("p");
  applyStyles(subtitle, PDF_STYLES.subtitle);
  subtitle.textContent = dateFrom && dateTo
    ? `Transaction Period: ${formatDate(dateFrom)} to ${formatDate(dateTo)}`
    : "Complete Report";
...
const createTableHeader = () => {
  ...
  const headers = ["Date", "Type", "Reference", "Account", "Amount"];
...
const createTableRow = (item, index) => {
  ...
  const rowStyle = index % 2 === 0 ? PDF_STYLES.tableRowEven : PDF_STYLES.tableRowOdd;
...
//   const summaryCards = [
//     { label: "Total Invoices", value: stats.totalInvoices.toString(), color: "#1e40af" },
//     { label: "Total Amount", value: `$${stats.totalAmount}`, color: "#059669" },
//     { label: "Outstanding Balance", value: `$${stats.totalBalanceToPay}`, color: "#dc2626" },
//     { label: "Total Paid", value: `$${stats.totalPaidAmount}`, color: "#059669" }
//   ];
...
export const handleDownloadBankPDF = async ({queryParams,setExportingPDF}) => {
  try {
    setExportingPDF(true);

    // Validate html2pdf availability
    if (typeof html2pdf === "undefined") {
      toast.error("PDF generation library not available. Please refresh and try again.");
      return;
    }
    const startDate = queryParams.get("dateFrom");
    const endDate = queryParams.get("dateTo");
    ...
    const pdfOptions = {
      margin: [15, 15, 15, 15],
      filename: `Transaction_Report${new Date().toISOString().split('T')[0]}.pdf`,
      image: { 
        type: "jpeg", 
        quality: 0.95 
      },
