...
import { downloadExpenses } from "../../service/expenseService";
...
const createHeader = (dateFrom, dateTo) => {
  ...
  subtitle.textContent =
    dateFrom && dateTo
      ...
      : "Complete Expense History Report";
...
const createTableHeader = () => {
  ...
  const headers = ["Description", "Department", "Category", "Date", "Amount"];
...
// Create table row
const createTableRow = (exp, index) => {
...
const createDataTable = (data) => {
  ...
  data.forEach((exp, index) => {
    const row = createTableRow(exp, index);
...
const createSummarySection = (sum) => {
  ...
  const summaryCards = [
    ...
    { label: "Total Amount", value: `${sum.totalAmount}`, color: "#059669" },
  ];
...
export const handleDownloadExpensePDF = async ({
  setExportingPDF,
  startDate,
  endDate,
  category,
  ...
  try {
    ...
    const dataTable = createDataTable(expenses);
